...
class Purchase extends MY_Controller
{
	...
	*/
	...
	public function purchaseItemReceive($purchase_id, $purchase_item_id, $product_id, $quantity, $warehouse_id){
		...
		else
		{
			if($this->purchase_model->deleteModel($id))
			{
				...
				foreach ($data['purchase_item'] as $row) {
					$this->purchase_model->restoreQuantity($id,$row->product_id,$data['purchase'][0]->warehouse_id,$row->quantity);
				}
	...
	*/
	public function editPurchase(){
		...
		else
		{
			...
			// print_r($php_data);
			// exit;
			if($this->purchase_model->editModel($id,$data,$invoice)){
				$log_data = array(
						'user_id'  => $this->session->userdata('user_id'),
						'table_id' => $id,
