...
class Sales extends MY_Controller
{
	...
	{
		if(!$this->user_model->has_permission("list_sales"))
		{
	...
	*/
	public function add()
	{
		if(!$this->user_model->has_permission("add_sales"))
		{
			$this->load->view('layout/restricted');	
		}
		else
		{
			...
			{
				$this->session->set_flashdata('no_sales_person', 'Kindly create Saler Person / Biller  for PRIMARY WAREHOUSE ');
	...
	*/
	...
	{
		if(!$this->user_model->has_permission("edit_sales"))
		{
			$this->load->view('layout/restricted');	
		}
		else
		{
			...
			$data['shipping_country_id']=  $data['data'][0]->shipping_country_id;
			// echo '<pre>';
			// print_r($data);
			// exit;
	...
	*/
	public function delete($id){
		if(!$this->user_model->has_permission("delete_sales"))
		{
			$this->load->view('layout/restricted');	
		}
		else
	...
	*/
	...
	{
		if(!$this->user_model->has_permission("email_sales"))
		{
			$this->load->view('layout/restricted');	
		}
		else
		{
			...
			// echo realpath(dirname(dirname(__DIR__)) . '/assets/uploads')."/".$data[0]->reference_no.'.pdf';
			// exit;
			if(!$mail->Send())
			{
	...
	*/
	public function receipt($id)
	{
		...
		$data['to_ledger'] = $this->ledger_model->get_single_record($this->branch_model->get_single_record($data['data'][0]->branch_id)->ledger_id);
