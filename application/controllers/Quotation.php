...
class Quotation extends MY_Controller
{
	...
	public function index(){
		...
		if(!$this->user_model->has_permission("list_quotation"))
		{
	...
	*/
	public function add(){
		if(!$this->user_model->has_permission("add_quotation"))
		{
			$this->load->view('layout/restricted');	
		}
		else
		{
			...
			// $branch_id  = $this->ledger_model->get_branch_from_ledger_id($data['user']->ledger_id);
			// $data['customer_account_group_id'] = $this->account_group_model->get_record_by_group_title_branch_id($branch_id,'Sundry Debitors')->id;
			$data['role_id'] =  $this->user_model->get_single_role_record_by_name('customer')->id;
	...
	public function addQuotation(){
		...
		else
		{
			...
			if($quotation_id = $this->quotation_model->addModel($data)){
				...
				$this->customer_model->addModel($customer_data);
	...
	*/
	public function edit($id){
		if(!$this->user_model->has_permission("edit_quotation"))
		{
			$this->load->view('layout/restricted');	
		}
		else
					...
					foreach ($js_data as $key => $value) {
						if($value=='delete'){
							...
							if($old_quantity = $this->quotation_model->checkProductInQuotation($id,$product_id)){
								$this->quotation_model->removeQuantity($id,$product_id);
							}
							
							$this->quotation_model->addQuotationItem($data,$product_id,$warehouse_id,$quantity);
	...
	*/
	public function delete($id){
		if(!$this->user_model->has_permission("delete_quotation"))
		{
			$this->load->view('layout/restricted');	
		}
		else
	...
	*/
	public function view($id){
		if(!$this->user_model->has_permission("view_quotation"))
		{
			$this->load->view('layout/restricted');	
		}
		else
	...
	*/
	public function pdf($id){
		if(!$this->user_model->has_permission("download_quotation"))
		{
			$this->load->view('layout/restricted');	
		}
		else
	...
	*/
	public function email($id){
		if(!$this->user_model->has_permission("email_quotation"))
		{
			$this->load->view('layout/restricted');	
		}
		else
		{
			...
			$quotation = $this->quotation_model->get_single_record_by_id($id);
			$user = $this->user_model->get_single_record_by_id($quotation->customer_id);
		//	$company = $this->purchase_model->getCompany();
			...
			$mail->SMTPAuth = true;
			$mail->SMTPSecure = $email->email_encription;
			...
			$mail->Subject = "Quotation order No : ".$quotation->reference_no." From ".@$user->company->name;
			$mail->Body = "Date : ".$quotation->date."<br>Total : ".$quotation->total;
			$mail->AddAttachment(realpath(dirname(dirname(__DIR__)) . '/assets/uploads')."/".$quotation->reference_no.'.pdf');
			$mail_response = $mail->send();
			//$mail->AltBody = "This is the body in plain text for non-HTML mail clients";

			if(!$mail_response)
			{
				$message =  "Email could not be sent " . $mail->ErrorInfo . json_encode($email);
			}
			else{
