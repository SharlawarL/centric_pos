  ...
  //       }
	...
	public function permission()
	{
        ...
        else
        {
			...
			$data['plan_permissions']				= $this->user_model->get_permission_records_by_module("plan");
			$data['license_permissions']			= $this->user_model->get_permission_records_by_module("license");
	...
	public function add_role()
	{
		...
		else
		{
			...
			if($_POST)
			{
				...
				$this->form_validation->set_rules('name', 'Name', 'trim|required|min_length[3]|is_unique[groups.name]',array('is_unique' => 'The %s is already taken'));
			...
			if($_POST)
			{
		        ...
		        else
		        {
		    		...
		    		if($this->user_model->edit_role_record($data,$id)){ 
		    			$this->session->set_flashdata('success', 'New Role has been updated successfully.');
						redirect("user",'refresh');
		    		}
		        }
