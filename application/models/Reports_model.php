...
class Reports_model extends CI_Model
{
	...
	public function getReceivableAmountDetails($start_date,$end_date,$customer_id){
		return $this->db->query('
											SELECT 
												i.*,
	...
	public function getPayableAmountDetails($start_date,$end_date,$supplier_id){
		return $this->db->query('
											SELECT 
												ar.*,
												s.first_name as supplier_name,
	...
	*/
	...
	public function createSST($data)
	{
		if($this->db->insert('sst', $data)){
	...
	public function getSSTDetail($id){
		...
		$sql = "SELECT * FROM `sst` WHERE sst_id = $sst_id";
		$data = $this->db->query($sql);
		return $data->result();
		
	}

	public function updateSST($id, $data)
	{
		$this->db->where('sst_id',$id);
		if($this->db->update('sst',$data)){
			return TRUE;
		}
		else{
			return FALSE;
	...
	public function deleteSST($id)
	{
		$this->db->where('sst_id',$id);
		if($this->db->update('sst',array('delete_status'=>1))){
			return TRUE;
		}
		else {
			return FALSE;
	...
	public function submitSST($data)
	{
		$this->db->insert('sst_form', $data);
		$id =  $this->db->insert_id();
		$this->db->where('sst_id', $data['sst_id']);
		if($this->db->update('sst',array('sst_status'=>1))){
			return $id;
		}
		else {
			return FALSE;
	...
	public function getSSTFormDetails($id)
	{
		$sst_id = null;
		if ($id) {
			$sst_id = $id;
		...
		$sql = "SELECT * FROM `sst_form` WHERE sst_id = $sst_id";
		$data = $this->db->query($sql);
		return $data->result();
	}
}
