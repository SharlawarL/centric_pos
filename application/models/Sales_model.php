...
class Sales_model extends CI_Model
{
	...
	*/
	public function getSales()
	{
		...
		if($this->session->userdata('type') == "admin"){
			$this->db->select('s.*,c.*,i.*,b.first_name as biller_first_name, b.last_name as biller_last_name')
			         ->from('sales s')
			         ->join('users b','s.biller_id=b.id')
			         ->join('users c','s.customer_id=c.id')
			...
			$data = $this->db->select('s.*,bc.first_name as biller_first_name,bc.last_name as biller_last_name,cc.id as customer_id,cc.first_name as customer_first_name,cc.last_name as customer_last_name,i.*')
			         ->from('sales s')	
			         ->join('invoice i ','i.sales_id = s.sales_id')
			         ->join('users bc','bc.id = s.biller_id')
			         ->join('users cc','cc.id = s.customer_id')
			         ->join('warehouse_management wm','wm.user_id = s.biller_id')
	...
	*/
	public function getWarehouse(){
		if($this->session->userdata('type') == "admin"){
			...
			$this->db->select('w.*,wm.user_id as user_id,b.branch_name as branch_name,u.first_name as biller_name,b.state_id as biller_state_id,u.id as biller ')
					 ->from('warehouse w')
	...
	*/
	public function getSalesWarehouse(){
		...
		if($this->session->userdata('type') == "admin")
		{
			...
			$sales_user_id = $this->db->select('w.*,wm.user_id as user_id,b.branch_name as branch_name,u.first_name as biller_name,b.state_id as biller_state_id,u.id as biller')
					 ->from('warehouse w')
					 ->join('warehouse_management wm','wm.warehouse_id = w.warehouse_id')
	...
	*/
	public function getBillerWarehouseDetails($user_id){
		...
		$biller_warehouse = $this->db->select('w.*,wm.user_id as user_id,u.id as biller_id,br.state_id as biller_state_id')
				 ->from('warehouse w')
				 ->join('warehouse_management wm','wm.warehouse_id = w.warehouse_id')
				 ->join('users u','wm.user_id = u.id')
	...
	*/
	public function getCustomerStateId($customer_id){
		...
		$result = $this->db->select('state_id,country_id')
		         ->from('users')
	...
	*/
	public function getBiller()
	{
		...
		{
			return $this->db->select('u.*,b.state_id as biller_state_id,w.warehouse_id as warehouse,w.warehouse_name as warehouse_name')
		         ->from('users u')
		         ->join('warehouse_management wm','wm.user_id = u.id')
	...
	*/
	public function getProduct($product_id,$warehouse_id){
		return $this->db->select('p.product_id,p.code,p.hsn_sac_code,p.unit,p.name,p.size,p.cost,p.price,p.alert_quantity,p.image,p.category_id,p.subcategory_id,p.tax_id,wp.quantity,wp.warehouse_id')
			 ->from('products p')
	...
	*/
	public function getDetails($id){
			...
			// 						   i.invoice_date,
			// 						   i.paid_amount,
			// 						   CONCATE(c.first_name," ",c.last_name) as customer_name,
			...
			// 						   CONCATE(b.first_name," ",b.last_name) as biller_name,
			// 						   b.phone as biller_mobile,
			// 						   b.email as biller_email,
			// 						   b.company as biller_company,									   
			...
			return  $this->db->select('s.*,
							 ...
							 ->join('states sssh','sssh.id = s.shipping_state_id')
							 ->join('countries csh','csh.id = s.shipping_country_id')
							 ->join('transaction_header t','t.invoice_id = i.id','left')
							 ->where('s.sales_id',$id)
							 ->get()
							 ->result();
		// echo $this->db->last_query();
	...
	*/
	public function getDetailsReceipt($id){
			...
			return  $this->db->select('s.*,
						 ...
						 ->join('cities cr','br.city_id = cr.id','left')
						 ->where('s.sales_id',$id)
						 ->get()
						 ->result();
	...
	*/
	public function invoice(){
		return $this->db->select('*,CONCATE(c.first_name," ",c.last_name) as customer_name')
					    ->from('invoice i')
	...
	*/
	public function getCustomerData($id){
		$this->db->where('id',$id);
		return $this->db->get_where('users')->row();
	}
	/*
		return country
	*/
	...
	*/ 
	public function getBillerWithAllData($biller_id){
		return  $this->db->select('s.*,
								   ...
								   cs.name as customer_state,
								   cc.name as customer_country,
								   b.first_name as biller_first_name,
								   b.last_name as biller_last_name,
								   b.address as biller_address,
								   cb.name as biller_city,
