...
class Purchase_model extends CI_Model
{
	...
	*/
	...
	public function restoreQuantity($id,$product_id,$warehouse_id,$quantity){
		$item_quantity = $this->db->get_where('purchase_items',array('product_id'=>$product_id,"purchase_id"=>$id))->row()->quantity;
	...
	*/
	public function getBarcodeProducts($term){
		return  $this->db->select('product_id,code,name,barcode_id')
				        ->from('products')
				        ->like('code',$term)
				        ->or_like('name',$term)
				        ->or_like('barcode_id',$term)
				        ->or_like('sku',$term)
