...
class Ion_auth_model extends CI_Model
{
	...
	public function __construct()
	{
		...
		$this->load->model(array('ledger_model'));
	...
	**/
	...
	public function allReports(){
		...
		// $data['supplier'] = $this->db->query('SELECT count(*) as count FROM users as u JOIN users_groups as ug ON u.id = ug.user_id JOIN groups as g ON g.id = ug.group_id WHERE g.name = "supplier"')->row();
		$this->db->select('count(*) as count');
		$this->db->join('users_groups ug','u.id = ug.user_id');
		$this->db->join('groups g','g.id = ug.group_id');
		$this->db->where('g.name','supplier');
		$data['supplier'] = $this->db->get('users u')->row();
		$data['customer']  = $this->db->query('SELECT count(*) as count FROM users u JOIN users_groups ug ON u.id = ug.user_id JOIN groups g ON g.id = ug.group_id WHERE g.name = "customer"')->row();
		$this->db->select('count(*) as count');
		$this->db->join('users_groups ug','u.id = ug.user_id');
		$this->db->join('groups g','g.id = ug.group_id');
		$this->db->where('g.name','customer');
		$data['supplier'] = $this->db->get('users u')->row();

		// $data['biller']    = $this->db->query('SELECT count(*) as count FROM users u JOIN users_groups ug ON u.id = ug.user_id JOIN groups g ON g.id = ug.group_id WHERE g.name = "sales_person"')->row();;
		$this->db->select('count(*) as count');
		$this->db->join('users_groups ug','u.id = ug.user_id');
		$this->db->join('groups g','g.id = ug.group_id');
		$this->db->where('g.name','sales_person');
		$data['biller'] = $this->db->get('users u')->row();

		$data['urgent_product'] = $this->db->select('w.*,wp.quantity as wp_quantity,p.name as product_name')
											 ->from('warehouse w')
											 ->join('warehouses_products wp','w.warehouse_id = wp.warehouse_id')
											 ->join('products p','wp.product_id = p.product_id')
											 // ->where('p.user_id',$this->session->userdata('user_id'))
