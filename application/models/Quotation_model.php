...
class Quotation_model extends CI_Model
{
	...
	*/
	public function getQuotation()
	{
		if($this->session->userdata('type') == "admin"){
			$this->db->select('s.*,b.first_name as biller_first_name, b.last_name as biller_last_name,c.first_name as customer_first_name, c.last_name as customer_last_name')
			         ->from('quotation s')
		...
		{
			$data = $this->db->select('s.*,bc.first_name as biller_first_name,bc.last_name as biller_last_name,cc.id as customer_id,cc.first_name as customer_first_name,cc.last_name as customer_last_name')
			         ->from('quotation s')	
			         ->join('users bc','s.biller_id=bc.id')
		         	 ->join('users cc','s.customer_id=cc.id')			         
		...
		//          ->from('quotation q')
		//          ->join('user b','q.biller_id=b.id')
		//          ->join('user c','q.customer_id=c.id')
	...
	*/
	public function getBiller(){
		return $this->db->get_where('users',array('delete_status'=>0))->result();
	}
	/* 
	...
	*/
	public function getRecord($id){
		...
		return $this->db->select('
								...
								')
						 ->from('quotation q')
						 ->join('users c','c.id = q.customer_id')
	...
	*/
	...
	public function removeQuantity($quotation_id,$product_id){
		$where = "quotation_id = $quotation_id AND product_id = $product_id";
		$this->db->where($where);
	...
	*/
	public function getDetails($id){
		...
		$result = $this->db->select('q.*,
									...
									bl.first_name as biller_first_name,
									bl.last_name as biller_last_name,
									b.address as biller_address,
									ccs.name as biller_city,
									css.name as biller_state,									
									ccrs.name as biller_country,
									bl.phone as biller_mobile,
									bl.email as biller_email,
									qcs.name as shipping_city,
									qss.name as shipping_state,
									qcrs.name as shipping_country,
									b.gstin as biller_gstid
								 ')
						 ->from('quotation q')
						 ->join('warehouse w','w.warehouse_id = q.warehouse_id')
						 ->join('branch b','b.branch_id = w.branch_id')
						 ->join('users bl','bl.id = q.biller_id')
						 ->join('users c','q.customer_id = c.id')
						 ->join('cities ccs','ccs.id = c.city_id')
						 ->join('states css','css.id = c.state_id')
						 ->join('countries ccrs','ccrs.id = c.country_id')
	...
	*/
	...
	public function get_single_record_by_id($id) {
