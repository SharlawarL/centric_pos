...
<script>
    $('#branch').change(function(){
      var id = $(this).val();
      $.ajax({
          url: "<?php echo base_url('payment/getAccount') ?>/"+id,
          type: "GET",
          dataType: "JSON",
          success: function(data){

            var from_ac = data['from_ac'];
            var to_ac = data['to_ac'];
            //alert(from_ac[0].title);
            $('#from_ac').html('');
            $('#to_ac').html('');
            $('#from_ac').append('<option>Select</option>');
            $('#to_ac').append('<option>Select</option>');
            for(i=0;i<from_ac.length;i++){
              $('#from_ac').append('<option value="' + from_ac[i].id + '">' + from_ac[i].title +' ('+from_ac[i].group_title+ ')' + '</option>');
            }
            for(i=0;i<to_ac.length;i++){
               $('#to_ac').append('<option value="' + to_ac[i].id + '">' + to_ac[i].title +' ('+to_ac[i].group_title+ ')' + '</option>');
            }
          }
        });
    });
