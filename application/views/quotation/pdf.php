...
<body>
	...
	<table width="100%" border="1" cellspacing="0" style="border: 0px solid black; border-collapse: collapse;" class="table" cellpadding="2">
			<tr>
				<td colspan="6" style="border: 0px;text-align: right;">Quotation</td>
		...
		<tr>
			<td valign="top" rowspan="" style="text-align:left;font-size: 13px;padding: 10px;" colspan="4" rowspan="5">
					...
					<?php echo $data[0]->customer_first_name.' '.$data[0]->customer_last_name;?>
					<br/>
		...
		<tr>
			...
			<th style="text-align: center;font-size: 12px;" colspan="4">Subtotal</th>
		</tr>
			<?php 
			...
			<tr>
				...
				<td align="right" class="fontS" colspan="2"><?php echo $value->discount_value;?></td>
				<td align="right" class="fontS" colspan="2"><?php echo number_format($value->gross_total-$value->discount,2, '.', ',');?></td>
				...
				<td align="right" class="fontS" colspan="4"><?php echo number_format($value->gross_total - $value->discount + $value->igst_tax + $value->cgst_tax + $value->sgst_tax,2, '.', ','); ?></td>
			</tr>
			<?php 
					$i++;
			...
			<tr>
				...
				<td align="right" class="fontS" colspan="2">
					<?php
						echo number_format($data[0]->discount_value,2, '.', ',');
					?>
				</td>
				<td align="right" class="fontS" colspan="2">
					<?php
						echo number_format($data[0]->total-$data[0]->tax_value,2, '.', ',');
					?>	
				</td>
				...
				<td align="right" class="fontS" colspan="4">
					<?php
						echo number_format($data[0]->total,2, '.', ',');
					?>		
				</td>
			...
			<tr>
				...
				<td align="right" colspan="2" class="fontH"><?php echo number_format($data[0]->total+$data[0]->shipping_charge-$data[0]->flat_discount,2, '.', ','); ?></td>
			</tr>
			<!-- <tr>
				<td colspan="12" align="right" class="footerpad fontH"><b>Due Amount</b></td>
				<td align="right" colspan="2" class="fontH">
					<?php
						echo number_format($data[0]->shipping_charge+$data[0]->total-$data[0]->flat_discount,2, '.', ',');
					?>
				</td>
			</tr> -->
        <tr>
        	<td colspan="14" valign="top" style="border-bottom: 0px;font-size: 13px;">
        		Paid Amount (in Words)<br>
        		<b> RM <?php echo $this->numbertowords->convert_number(round($data[0]->shipping_charge+$data[0]->total-$data[0]->flat_discount)); ?> Only</b>
        	</td>
        </tr>
        <tr>
        	<td colspan="14" style="border-bottom: 0px;font-size: 13px;">
        		<b>Head Office : </b>
        			<span>
	...
	<table>
		<tr>
        	<td colspan="14" style="border:0px;text-align: center;font-size: 13px;"><b>This is Computer Generated Quotation</b></td>
        </tr>
	</table>
</body>
</html>
