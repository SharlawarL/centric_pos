...
<body>
	<table width="670px" border="1" cellspacing="0" style="border: 0px solid black; border-collapse: collapse;" class="table" cellpadding="2">
					...
					<tr>
						...
						<td style="border: 0px;font-size: 13px;padding-left: 10px;" valign="top">
							...
							<span style="font-size: 10px;">
								...
								<br>
								<?php if(isset($data[0]->biller_gstid)){echo "GSTIN : ".$data[0]->biller_gstid;}?>
							</span>
		...
		<tr style="font-size: 13px;">
			...
			<td valign="top" colspan="4" style="font-size: 13px;">
				Payment Mode <br>
				<p style="font-size: 13px;font-weight:bold"><?php if(isset($data[0]->mode)){echo $data[0]->mode;}?>  <?php // if(isset($data[0]->due_days)){echo $data[0]->due_days;}?></p>
			</td>
		</tr>
		...
		<tr>
			...
			<tr>
				...
				<?php 
					if($value->tax_type =="Inclusive"){
				?>
				<td align="center" class="fontS"><?php echo number_format($value->gross_total-$value->discount - $value->igst_tax -$value->cgst_tax -$value->sgst_tax,2, '.', ',');?></td>
				<?php 
					}else{
				?>
				<td align="center" class="fontS"><?php echo number_format($value->gross_total-$value->discount,2, '.', ',') ;?></td>
				<?php
					}
				?>
				...
				<td align="center" class="fontS" colspan="2">
					<?php 
						...
						if($value->tax_type =="Inclusive"){
							echo number_format($value->gross_total - $value->discount - $value->igst_tax - $value->cgst_tax - $value->sgst_tax,2, '.', ',');
						}else{
							echo number_format($value->gross_total - $value->discount,2, '.', ',') ;
						}	
			...
			<tr>
				...
				<td align="center" class="fontS" colspan="2">
					<?php
						echo number_format($data[0]->total-$data[0]->tax_value-$data[0]->shipping_charge+$data[0]->flat_discount,2, '.', ',');
					?>	
				</td>
				<!-- <td align="right" class="fontS">
					<?php echo $igst;?>	
				...
				<td align="right" class="fontS" colspan="5">
					<?php
						...
						echo number_format($tot+$igst+$cgst+$sgst,2, '.', ',');
					?>		
				</td>
			...
			<tr>
				...
				<td align="right" colspan="2" class="fontH"><?php echo number_format($data[0]->shipping_charge,2, '.', ','); ?></td>
			</tr>
			<tr>
				<td colspan="12" align="right" class="footerpad fontH"><b>Total Discount</td>
				<td align="right" colspan="2" class="fontH"><?php echo number_format($data[0]->discount_value+$data[0]->flat_discount,2, '.', ','); ?></td>
			</tr>
			<!-- <tr>
			...
			<tr>
				...
				<td align="right" colspan="2" class="fontH"><?php echo number_format($tot+$igst+$cgst+$sgst+$data[0]->shipping_charge-$data[0]->flat_discount,2, '.', ','); ?></td>
			</tr>
			<tr>
				...
				<td align="right" colspan="2" class="fontH">
					<?php
						if($data[0]->paid_amount!=null){
							echo number_format($data[0]->paid_amount,2, '.', ',');
						}
						else{
							echo "0";
						}
			...
			<tr>
				...
				<td align="right" colspan="2" class="fontH">
					<?php
						echo number_format($tot+$igst+$cgst+$sgst+$data[0]->shipping_charge-$data[0]->paid_amount-$data[0]->flat_discount,2,'.',',');
					?>
				</td>
			</tr>
        <tr>
        	<td colspan="7" valign="top" style="border-bottom: 0px;font-size: 13px;">
        		...
        		<b> RM <?php echo $this->numbertowords->convert_number(round($tot+$igst+$cgst+$sgst+$data[0]->shipping_charge-$data[0]->paid_amount-$data[0]->flat_discount)); ?> Only</b><br/>
        	</td>
        	<td colspan="7" valign="top" style="border-bottom: 0px;font-size: 13px;">
        		Paid Amount (in Words)<br>
        		<b> RM <?php echo $this->numbertowords->convert_number(round($data[0]->paid_amount+$data[0]->flat_discount)); ?> Only</b>
        	</td>
